...
class ViewController: UIViewController {
    ...
    let minimalDistance = CGFloat(40)
...
extension ViewController {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let distance = sqrt(
            ...
            guard minimalDistance < distance else { return }
        }
        addNode(node, to: point)
    }
...
extension ViewController: ARSCNViewDelegate {
    ...
    func createFloor(anchor: ARPlaneAnchor) -> SCNNode {
        let wight = CGFloat(anchor.extent.x)
        let height = CGFloat(anchor.extent.y)
        
        let node = SCNNode(geometry: SCNPlane(width: wight, height: height))
